...
1. Use Node 20 or newer (`package.json` declares the supported range), enable Corepack, then run `pnpm install --frozen-lockfile`.
...
Vercel Preview and Development deployments use sample data when Sheets credentials are unavailable, so pull requests remain deployable without exposing production secrets. Vercel Production deployments still fail without Sheets credentials and never opt into sample data automatically. Do not set `ALLOW_SAMPLE_DATA=true` in Vercel Production.
